    ...
    <div className="card">
      ...
      <Modal open={open} onClose={()=>setOpen(false)} title={`Edit ${entity}`} footer={<>
        ...
        {entity==='item' && (<>
          ...
          <div className="field"><label>Tagged category (auto if category is tagged)</label><input className="input" value={payload.is_valuable?'Yes':'No'} disabled/></div>
          <div className="field"><label>Article number (required if Tagged)</label><input className="input" value={payload.article_number||''} onChange={e=>setPayload({...payload, article_number:e.target.value})}/></div>
        </>)}
      </Modal>
